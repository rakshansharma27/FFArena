...
CREATE TABLE public.tournaments (
    ...
    entry_fee_paise BIGINT DEFAULT 0 NOT NULL CHECK (entry_fee_paise >= 0),
...
CREATE TABLE public.sponsorship_deals (
    ...
    amount_paise BIGINT NOT NULL CHECK (amount_paise >= 200000),
...
CREATE TABLE public.prize_transactions (
    ...
    tds_deducted_paise BIGINT NOT NULL,
...
CREATE POLICY "Organizers can insert own tournaments" ON public.tournaments FOR INSERT WITH CHECK (
    auth.uid() = organizer_id 
    AND EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('ORGANIZER', 'ADMIN'))
);
CREATE POLICY "Organizers can update own tournaments" ON public.tournaments FOR UPDATE USING (
    auth.uid() = organizer_id 
    OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'ADMIN')
);

CREATE POLICY "Teams are viewable by everyone" ON public.teams FOR SELECT USING (true);
CREATE POLICY "Captains can insert team" ON public.teams FOR INSERT WITH CHECK (auth.uid() = captain_id);
CREATE POLICY "Captains can update own team" ON public.teams FOR UPDATE USING (auth.uid() = captain_id);

CREATE POLICY "Matches are viewable by everyone" ON public.matches FOR SELECT USING (true);
CREATE POLICY "Organizers of tournament can edit matches" ON public.matches FOR UPDATE USING (
    EXISTS (
...
CREATE POLICY "Users can view own transactions" ON public.prize_transactions FOR SELECT USING (
    player_id = auth.uid() 
    OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'ADMIN')
);
...
-- Leaderboard Updater Trigger Function
...
CREATE OR REPLACE TRIGGER trigger_update_leaderboard_on_match_resolve
